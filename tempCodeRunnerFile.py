...
class player :
    ...
    #called when the player takes a yellow card (the 3 will be red)
...
class matchAi :
    ...
    def decide_action(self, state):
        ...
        if min(state["stamina"]) < 50 and state["minute"] >= 60:
            return "SUBSTITUTE"
        ...
        #if loosing  then the formation must be attacking and we need to push_attack
        if opponent_score > my_score:
            if state["minute"] >= 75:
                if self.controlled_team.formation != "ATTACKING":
                    return "CHANGE_TO_ATTACKING"
